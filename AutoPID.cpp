...
void AutoPID::run(unsigned long systime) {
  if (_stopped) {
    _stopped = false;
    reset(systime);
  }
  ...
  if (_bangOn && ((*_setpoint - *_input) > _bangOn)) {
    ...
    *_output = _outputMin;
    ...
    unsigned long _dT = systime - _lastStep;   //calculate time since last update
    if (_dT >= _timeStep) {                     //if long enough, do PID calculations
      ...
      if (PID < _outputMin) { *_output = _outputMin;}
      else if (PID > _outputMax) { *_output = _outputMax;}
      else {*_output = PID;}
    }
  }
}//void AutoPID::run

void AutoPID::stop(unsigned long systime) {
  _stopped = true;
  reset(systime);
}
void AutoPID::reset(unsigned long systime) {
  _lastStep = systime;
  _integral = 0;
  _previousError = 0;
}
...
void AutoPIDRelay::run(unsigned long systime) {
  AutoPID::run(systime);
  while ((systime - _lastPulseTime) > _pulseWidth) _lastPulseTime += _pulseWidth;
  *_relayState = ((systime - _lastPulseTime) < (_pulseValue * _pulseWidth));
}
